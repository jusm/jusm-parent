...
public class JwtAccessDecisionManager implements AccessDecisionManager {
	...
	public void decide(Authentication authentication, Object object, Collection<ConfigAttribute> configAttributes)
		...
		//期望的权限 基于角色也就是期望的角色
		final StringBuilder expectAuthorities = new StringBuilder("[");
		//实际的权限 基于角色也就是实际的角色
		final StringBuilder actualAuthorities = new StringBuilder("[");
		boolean flag = false;
		Iterator<ConfigAttribute> ite = configAttributes.iterator();
		while (ite.hasNext()) {
			ConfigAttribute ca = ite.next();
			String needRole = ((SecurityConfig) ca).getAttribute();
			expectAuthorities.append(needRole.trim()).append(" ");
			// ga 为用户所被赋予的权限。 needRole 为访问相应的资源应该具有的权限。
			for (GrantedAuthority ga : authentication.getAuthorities()) {
				if (!flag) {
					actualAuthorities.append(ga.getAuthority().trim()).append(" ");
				}
				if (needRole.trim().equalsIgnoreCase(ga.getAuthority().trim())) {
					logAccessDecision(authentication, object, expectAuthorities, actualAuthorities);
					return;
				}
			}
			flag = true;
	 ...
	 * @param object
	 * @param expectAuthorities
	 * @param actualAuthorities
	 */
	private void logAccessDecision(Authentication authentication, Object object, final StringBuilder expectAuthorities,
			final StringBuilder actualAuthorities) {
		Object principal = authentication.getPrincipal();
		if(principal != null && principal instanceof User) {
			User user = (User)principal;
			logger.info("Request url: " + object + " need "+expectAuthorities.append("]") +" and user:'"+user.getUsername()+"' actually has "+actualAuthorities.append("]"));
		}else {
			logger.info("Request url: " + object + " need "+expectAuthorities.append("]") +" and user actually has "+actualAuthorities.append("]"));
		}
	}
